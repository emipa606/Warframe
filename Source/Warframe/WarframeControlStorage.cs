...
namespace Warframe
{
    ...
    {
        public Dictionary<Pawn, Building_ControlCell> BeControlerAndControlCell =
            new Dictionary<Pawn, Building_ControlCell>();

        public Dictionary<Building_ControlCell, Pawn> ControlCellAndBeControler =
            new Dictionary<Building_ControlCell, Pawn>();
        ...
        {
            base.ExposeData();
            Scribe_Collections.Look(ref ControlCellAndBeControler, "ControlCellAndBeControler", LookMode.Reference,
                LookMode.Reference);
            Scribe_Collections.Look(ref BeControlerAndControlCell, "BeControlerAndControlCell", LookMode.Reference,
                LookMode.Reference);
        ...
        */
        ...
        public bool checkBeControlerExist(Pawn becontroler)
        {
            ...
            var build = BeControlerAndControlCell.TryGetValue(becontroler);
            if (!becontroler.Spawned && !wfSpawned(becontroler) || becontroler.Dead)
            {
                remove2pawnRelation(build, becontroler);
                return false;
            }

            if (build is {Spawned: true, Destroyed: false})
            {
                return true;
            }
